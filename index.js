...
const parseLink = function (link) {
    ...
    return {
        id: match?.[8] != null ? +match[8] : null, server: match?.[9] != null ? {
            ip: match[10], port: match?.[11] != null ? +match[11] : null
        } : null
    }
}
...
const getSystemInfo = function (url = String(), options = {
                ...
                try {
                    ...
                    socket.on('message', function (message, isBinary) {
                        ...
                        if (!isBinary) try {
                            ...
                            } else if (data.name === "cannot_join") setError("Could not join the requested game. Perhaps the game is full/empty or has expired?"); else if (data.name === "player_name") output.players.push(data.data)
                        } catch (e) {
                        }
                    });
